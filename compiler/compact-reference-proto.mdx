...
@(anchor-here id module-name function-name struct-name enum-name contract-name tvar-name type-name)
...
@(request-snippet gparams generic-param)
...
@(request-snippet type tref tsize)
...
@(request-snippet struct-declaration)
...
@(request-snippet enum-declaration)
...
@(anchor-here contract-declaration circuit-declaration)
...
@(request-snippet type-alias-declaration)
...
@(request-snippet pattern pattern-struct-elt)
...
@(request-snippet program program-element)
...
@(request-snippet module-definition)
...
@(request-snippet witness-declaration)
...
@(request-snippet ledger-declaration)
...
@(request-snippet constructor-definition)
...
@(request-snippet circuit-definition)
...
the enclosing exported circuit's return value, where it can be read
from TypeScript via the `events` field of `CircuitResults` (see
...
@(parameter max-unsigned).
This value is 256<sup>k</sup>-1 where *k* is the number of bytes (@(parameter field-bytes)) that fits in
a `Field`.
- The maximum length of a vector or byte vector is @(parameter max-bytes/vector-length).
